  ...
  <div id="filter-contain">
    <div class="RadioContain">
      ...
      <input
        ...
        v-model="type"
        @change="clickedMy"
        title="Search new videos from subscriptions"
      />
      <label for="typeMy" title="Search new videos from subscriptions"
      ...
      <input
        ...
        value="channel"
        v-model="type"
        title="Search by channel username"
      />
      <label for="typeChannel" title="Search by channel username"
        >Channel</label
      >
      ...
      <div class="FilterSub">
        ...
        <div class="FilterRadioContain">
          <input
            ...
            value="relevance"
            v-model="filterSort"
            :disabled="filterSortDisabled"
            title="Resources are sorted based on their relevance to the search query."
          />
          <label
            for="relevanceSort"
            title="Resources are sorted based on their relevance to the search query."
          ...
          <input
            ...
            v-model="filterSort"
            :disabled="filterSortDisabled"
            title="Resources are sorted from highest to lowest rating."
          />
          <label
            for="ratingSort"
            title="Resources are sorted from highest to lowest rating."
          ...
          <input
            ...
            v-model="filterSort"
            :disabled="filterSortDisabled"
            title="Resources are sorted in reverse chronological order based on the date they were created"
          />
          <label
            for="dateSort"
            title="Resources are sorted in reverse chronological order based on the date they were created"
          ...
          <input
            ...
            v-model="filterSort"
            :disabled="filterSortDisabled"
            title="Resources are sorted from highest to lowest number of views."
          />
          <label
            for="viewSort"
            title="Resources are sorted from highest to lowest number of views."
          ...
          <input
            ...
            title="Channels are sorted in descending order of their number of uploaded videos"
          />
          <label
            for="videoSort"
            title="Channels are sorted in descending order of their number of uploaded videos"
            >Channel video count</label
          >
        </div>
      </div>
    ...
    <div class="FilterExtra">
      ...
      <div class="filter-row">
        <input
          ...
          step="1"
          v-model="maxResults"
          :disabled="maxResultsDisabled"
        />
        <span>{{ maxResults }}</span>
      </div>
      <div class="filter-row">
        <button
          class="filter-safe-button"
          :disabled="filterSafeDisabled"
          @click="toggleSafeFilter"
          :on="filterSafe === 'strict' ? true : false"
        >
          Child Filter {{ filterSafe === "strict" ? "On" : "Off" }}
        </button>
      </div>
      <div class="filter-row">
        <div
          class="filter-keyword-dropdown"
          :disabled="filterKeywordDisabled"
          @click="toggleKeywordFilter"
        >
          Advanced
          <icon
            class="keyword-dropdown-icon"
            :name="advancedMenuOpen ? 'arrow-up' : 'arrow-down'"
