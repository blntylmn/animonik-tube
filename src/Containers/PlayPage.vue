    ...
    <div class="sub-pick-contiain" v-if="type == 'channel' && !picked">
      ...
      <ul class="sub-pick-list">
        <li
          ...
          :key="channel.etag"
          @click="pickChannel(channel.id.channelId)"
        >
    ...
    <div class="sub-pick-contiain" v-if="type == 'playlist' && !picked">
      ...
      <ul class="sub-pick-list">
        <li
          ...
          :key="playlist.etag"
          @click="pickPlaylist(playlist.id.playlistId)"
        >
...
export default {
  ...
  methods: {
    ...
    pickChannel(channelId) {
      ...
      this.axios
        .get(
          `${global.YOUTUBE_ROOT}/search?part=snippet&channelId=${channelId}&maxResults=${this.maxResults}&type=video&order=${this.filterSort}&safeSearch=${this.filterSafe}&key=${process.env.API_KEY}`
        )
        .then(res => {
      ...
      //   .then(channel => {
      //     if (channel)
      //       // console.log(`The channel's title is ${channel.relatedPlaylists}`);
    ...
    pickPlaylist(playlist) {
      ...
      this.axios
        .get(
          `${global.YOUTUBE_ROOT}/playlistItems?part=snippet&playlistId=${playlist}&maxResults=${this.maxResults}&key=${process.env.API_KEY}`
        )
        .then(res => {
